// Endfield DX11 1.5: derived from this shader's own 0xDC23C161 dump.
#include "../shared.h"
#include "../common.hlsl"
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r4.xyz = t0.SampleLevel(s1_s, r2.xy, 0).xyz;
  r3.xyz = t0.SampleLevel(s1_s, r3.zw, 0).xyz;
  r2.xyz = t0.SampleLevel(s1_s, r2.zw, 0).xyz;
  r2.w = r1.x * 0.5 + r1.y;
  r2.w = r1.z * 0.5 + r2.w;
  r3.w = r4.x * 0.5 + r4.y;
  r3.w = r4.z * 0.5 + r3.w;
  ...
  r0.w = cb1[2].y * r0.w;
  r0.w = exp2(r0.w);
  // RenoDX: match the existing post-tone-map vignette control.
  float vignette_value = lerp(1.0f, r0.w, VIGNETTE_STRENGTH);
  r0.w = 1.0f;
  r1.xyz = -cb1[4].zxy + float3(1,1,1);
  r1.xyz = r0.www * r1.xyz + cb1[4].zxy;
  r0.xyz = r1.xyz * r0.xyz;
  r0.xyz = cb1[7].www * r0.xyz;
  /* Native 0xDC23C161: LUT shaper, lookup, sRGB output and fixed dither replaced below.
  ...
  r2.xyz = t2.SampleLevel(s1_s, r1.xz, 0).xyz;
  r3.x = cb1[7].y;
  r3.y = 0;
  r0.zw = r3.xy + r1.xz;
  r1.xyz = t2.SampleLevel(s1_s, r0.zw, 0).xyz;
  ...
  o0.w = min(1, r1.w);
  */
  // RenoDX code start: use the existing Endfield HDR/LUT path on pre-shaper scene color.
  ...
  if (CUSTOM_GRAIN_STRENGTH > 0.f) {
    o0.xyz = renodx::effects::ApplyFilmGrain(
        o0.xyz, v1.xy, CUSTOM_RANDOM, CUSTOM_GRAIN_STRENGTH * 0.03f);
  }
