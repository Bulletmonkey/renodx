// Endfield DX11 1.5: derived from this shader's own 0x0497F492 dump.
#include "../shared.h"
#include "../common.hlsl"
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.z = exp2(r0.z);
  r0.xy = r0.xy * r0.zz;
  // RenoDX: scale the shared displacement, including base offset and channel spread in both quality branches.
  r0.xy *= CHROMATIC_ABERRATION_STRENGTH;
  ...
  r2.w = r0.w * 4 + 1;
  r2.w = 1 / r2.w;
  r5.xy = -cb1[1].xy + v1.xy;
  r3.w = -cb1[2].x + 1;
  r3.w = cb1[1].w * r3.w + cb1[2].x;
  r5.xy = abs(r5.xy) * r3.ww;
  ...
  r5.x = 5 * r5.x;
  r5.y = saturate(r5.y * r4.w + r5.x);
  r4.w = cb0[27].z + -1;
  r4.w = cb1[2].w * r4.w + 1;
  r5.z = cb0[27].z * 0.5625 + -r4.w;
  r4.w = cb1[1].w * r5.z + r4.w;
  r5.x = saturate(r4.w * r3.w);
  r5.xy = log2(r5.xy);
  r5.xy = cb1[2].zz * r5.xy;
  r5.xy = exp2(r5.xy);
  ...
  /* Native 0x0497F492: LUT shaper, lookup, sRGB output and fixed dither replaced below.
  ...
  r2.xyz = t1.SampleLevel(s0_s, r1.xz, 0).xyz;
  r3.x = cb1[7].y;
  r3.y = 0;
  r0.zw = r3.xy + r1.xz;
