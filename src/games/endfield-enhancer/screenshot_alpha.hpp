...
// The native copy shader samples float4. Packed RGB supplied implicit alpha=1;
// preserve that contract when the photo intermediate gains a physical A channel.
inline std::vector<uint32_t> Patch(std::span<const uint8_t> bytes) {
  if (bytes.size() != 896 || bytes.size() % 4
  ...
  for (size_t i=5; i<words.size();) {
    ...
    if (op==22 && count==3 && words[i+2]==32) scalar=words[i+1]; // OpTypeFloat
    ...
    if (op==62) { // Exact shader has one store, directly from the sampled float4.
      if (store || count!=3 || !sampled || words[i+2]!=sampled) return {};
      store=i; output=words[i+1];
    }
    i+=count;
  }
  if (!scalar || !vector || !function || !store || !output) return {};
  const uint32_t one=words[3], opaque=one+1;
  // OpCompositeInsert changes only component 3; RGB operations and bindings stay identical.
...
// Driver calls never hold our state mutex. Native callbacks can run under
// ReShade/shared shader locks, so waiting on a driver-side lifetime lock here
...
struct Pending {
  ...
  bool CopyLayout(std::span<const api::pipeline_layout_param> source) {
    params.assign(source.begin(),source.end()); ranges.resize(params.size());
    for (size_t i=0;i<params.size();++i) {
      ...
      if (param.type==api::pipeline_layout_param_type::descriptor_table
        ...
        param.descriptor_table.ranges=ranges[i].data();
                 ...
                 && param.type!=api::pipeline_layout_param_type::push_descriptors) return false;
    }
    return !params.empty();
  }
  uint32_t count = 0;
  api::pipeline_subobject* objects = nullptr;
  ~Pending() { if (objects) renodx::utils::pipeline::DestroyPipelineSubobjects(objects,count); }
};
inline std::map<std::pair<api::device*,uint64_t>,std::shared_ptr<Pending>> pending_pipelines;
struct Cached { api::pipeline_layout layout{}; api::pipeline pipeline{}; api::pipeline_layout owned_layout{}; };
inline std::map<std::pair<api::device*,uint64_t>, Cached> pipelines;
inline std::map<api::device*,std::vector<Cached>> retired;
inline std::map<api::command_list*,bool> targets;
...
inline void OnBegin(api::command_list* cmd, uint32_t count,
  ...
  std::lock_guard lock(mutex);
  if (match) targets[cmd]=true; else targets.erase(cmd);
}
inline void OnEnd(api::command_list* cmd) { std::lock_guard lock(mutex); targets.erase(cmd); }
...
inline api::pipeline Prepare(api::device* device, api::pipeline_layout layout,
  ...
  for (uint32_t i=0; i<count; ++i) if (objects[i].type==api::pipeline_subobject_type::render_target_formats) {
  ...
  for (uint32_t i=0; i<count; ++i) if (objects[i].type==api::pipeline_subobject_type::blend_state
      && objects[i].count==1 && objects[i].data) {
    const auto& blend=*static_cast<const api::blend_desc*>(objects[i].data);
    // Changing source alpha must not change RGB through a source-alpha blend.
...
// Copy only CPU descriptions here. Pipeline initialization can run on the
// game's loader workers; never recursively enter driver pipeline creation there.
inline void OnInitPipeline(api::device* device, api::pipeline_layout layout, uint32_t count,
  ...
  if (!copied) return; // Refuse layouts we cannot reproduce compatibly.
  pending->layout=layout;pending->count=count;
...
inline api::pipeline PrepareOne(const std::pair<api::device*,uint64_t>& key) {
  ...
  // Use an independent, compatible layout. Destroying the game's layout while
  // this deferred job runs cannot invalidate the layout passed to the driver.
  ...
  if (found==pending_pipelines.end() || found->second!=pending || pipelines.contains(key)) {
    // Native destruction/handle reuse invalidated this job while the driver ran.
...
inline void PreparePending() {
  std::vector<std::pair<api::device*,uint64_t>> keys;
  { std::lock_guard lock(mutex); for (const auto& [key,pending]:pending_pipelines) keys.push_back(key); }
...
inline void OnDestroyLayout(api::device* device,api::pipeline_layout layout) {
  ...
  for (auto it=pipelines.begin();it!=pipelines.end();) {
    if (it->first.first!=device || it->second.layout!=layout) {++it;continue;}
    // A submitted command buffer may still reference the pipeline.
...
inline constexpr auto on_copy=[]<typename Context>(Context& context)
  ...
  // A native photo can create a new copy variant after the last present.
  // Resolve that exact variant before its first draw so alpha never falls back
  // to the source scene's zero alpha. Ordinary startup targets return above.
...
inline void OnDestroyDevice(api::device* device) {
  std::vector<Cached> destroy;
  {
    ...
    std::erase_if(targets,[&](const auto& entry){return entry.first->get_device()==device;});
    for (auto it=pipelines.begin();it!=pipelines.end();) {
    ...
    if (auto it=retired.find(device);it!=retired.end()) {
      destroy.insert(destroy.end(),it->second.begin(),it->second.end()); retired.erase(it);
    }
  }
...
inline void OnQueue(api::command_queue* q) { std::lock_guard lock(mutex); queues.insert(q); }
inline void OnDestroyQueue(api::command_queue* q) { std::lock_guard lock(mutex); queues.erase(q); }
inline void OnPresent() {
  if (registered) return;
  renodx::utils::command_action::Register(on_copy,{
      .shader_hash=0x105b9793u,.command_types=renodx::utils::command_action::COMMAND_TYPE_DIRECT_DRAW});
  registered=true;
}
inline void OnPhotoRequested() {
  // Called by the exact native photo allocator, before it enters Unity's
  // allocation code. Present also runs during loading and must never compile
  // photo pipelines while loader workers are creating/destroying native state.
  PreparePending();
}
inline void Use(DWORD reason) {
  if (reason==DLL_PROCESS_ATTACH) {
    ...
    renodx::utils::command_action::Unregister(on_copy); registered=false;
