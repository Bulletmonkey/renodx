...
// Same dedicated asset roles as the original renderer hiding. No character
// names or shader IDs; unknown roles use the bone-bound component classifier.
...
// Local head/neck skin removal is restricted to the body asset role. Clothing
// retains whole-component filtering so its neckline cannot be peeled away.
...
// Build a separate visible index buffer. Vertex data, skinning and the source
// indices are never changed. Keep index count/order so submesh ranges survive.
// Complete connected pieces are classified, avoiding sliced/deformed hoodies.
inline bool FilterHeadComponents(std::span<const Vertex> vertices,
  ...
  for (const auto& vertex : vertices) {
    uint32_t total = 0;
    for (float coordinate : vertex.position) if (!std::isfinite(coordinate)) return false;
  ...
  // Reconnect UV/normal seams only when both position AND skinning match.
  // Coincident parts with different animation influences remain independent.
  ...
  for (uint32_t i = 0; i < vertices.size(); ++i) {
    ...
    for (size_t j = 0; j < 4; ++j) {
      ...
      if (body_skin && weight && (head_bones[vertices[i].bones[j]] == kHeadBone
                                  || head_bones[vertices[i].bones[j]] == kNeckBone))
...
inline NeckFrame MakeNeckFrame(const std::array<float,16>& bind,
  ...
  for(float v:bind)if(!std::isfinite(v))return {};
  ...
  NeckFrame result;result.center=invert(neck);auto end=invert(head);
  for(int k=0;k<3;++k){if(!std::isfinite(result.center[k])||!std::isfinite(end[k]))return {};result.axis[k]=end[k]-result.center[k];result.length+=result.axis[k]*result.axis[k];}
  result.length=std::sqrt(result.length);if(result.length<1e-6)return {};
...
// Topology is examined across material/UV seams, but each resulting cap is
// assigned to a retained rim material. Existing vertices/skin data stay exact.
// Called only after FilterHeadComponents validates the decoded mesh.
inline std::vector<NeckCapTriangle> BuildNeckCaps(std::span<const Vertex> vertices,
  ...
  const auto near_neck=[&](const std::array<float,3>& p){
    ...
    for(int k=0;k<3;++k){double d=p[k]-frame.center[k]-height*frame.axis[k];radius+=d*d;}
  ...
  const auto key_position=[&](uint32_t i){
    std::array<double,3> key{};for(int k=0;k<3;++k)key[k]=std::round(vertices[i].position[k]/tolerance);return key;
  };
  std::sort(order.begin(),order.end(),[&](uint32_t a,uint32_t b){return key_position(a)<key_position(b);});
  for(size_t i=0;i<order.size();){
    size_t end=i+1;while(end<order.size()&&key_position(order[i])==key_position(order[end]))++end;
    for(size_t j=i;j<end;++j){
      canonical[order[j]]=order[j];
      if(end-i>64)continue;
      for(size_t k=i;k<j;++k){
        const auto& a=vertices[order[j]];const auto& b=vertices[canonical[order[k]]];
        std::array<std::pair<uint16_t,int>,8> weights{};
        for(int q=0;q<4;++q){weights[q]={a.bones[q],int(a.weights[q])};weights[q+4]={b.bones[q],-int(b.weights[q])};}
        std::sort(weights.begin(),weights.end());int distance=0,total=0;uint16_t bone=weights[0].first;
        for(const auto& weight:weights){if(weight.first!=bone){distance+=std::abs(total);total=0;bone=weight.first;}total+=weight.second;}
        distance+=std::abs(total);
        if(distance<=128){canonical[order[j]]=canonical[order[k]];break;}
      }
    }
    i=end;
  }
  struct Edge {uint32_t a,b,vertex;size_t triangle;bool removed;};
  ...
  for(size_t i=0;i<edges.size();){
    size_t end=i+1;while(end<edges.size()&&key(edges[i])==key(edges[end]))++end;
    // Only a manifold edge shared by retained and removed faces is a cut.
    if(end-i==2&&edges[i].a==edges[i+1].b&&edges[i].b==edges[i+1].a
      ...
      auto cut=edges[i];std::swap(cut.a,cut.b);cut.vertex=cut.a;boundary.push_back(cut);
    }
    i=end;
  }
  ...
  for(size_t i=0;i<boundary.size();++i){
    auto& e=boundary[i];next[e.a]=next[e.a]==-1?static_cast<int>(i):-2;++incoming[e.b];
  }
  std::vector<bool> visited(boundary.size());std::vector<NeckCapTriangle> result;
  for(size_t first=0;first<boundary.size();++first){
    if(visited[first])continue;
    std::vector<uint32_t> ring;
    size_t current=first;bool closed=false;
    while(!visited[current]&&ring.size()<256){
      visited[current]=true;const auto& e=boundary[current];
      if(next[e.a]<0||incoming[e.a]!=1||next[e.b]<0||incoming[e.b]!=1)break;
      ring.push_back(e.vertex);current=static_cast<size_t>(next[e.b]);
      if(current==first){closed=true;break;}
    }
    if(!closed||ring.size()<3)continue;
    // This is a neck cap, not a general hole-filler for eyes, sleeves or hems.
    bool neck=true;
    for(auto index:ring){
      bool bound=false;
      for(size_t j=0;j<4;++j)if(vertices[index].weights[j]&&vertices[index].bones[j]<roles.size()
    ...
    if(framed){
      ...
      bool local=true;
      for(auto index:ring){local &= near_neck(vertices[index].position);for(int k=0;k<3;++k)center[k]+=vertices[index].position[k]/float(ring.size());}
      double height=0,radius=0;
      for(int k=0;k<3;++k)height+=(center[k]-frame.center[k])*frame.axis[k];
      for(int k=0;k<3;++k){double d=center[k]-frame.center[k]-height*frame.axis[k];radius+=d*d;}
      if(!local||height< -frame.length||height>1.25*frame.length||radius>frame.length*frame.length*.64)continue;
    }else if(!neck)continue;
    std::array<double,3> normal{};
    for(size_t i=0;i<ring.size();++i){
      const auto& a=vertices[ring[i]].position;const auto& b=vertices[ring[(i+1)%ring.size()]].position;
      normal[0]+=(a[1]-b[1])*(a[2]+b[2]);normal[1]+=(a[2]-b[2])*(a[0]+b[0]);normal[2]+=(a[0]-b[0])*(a[1]+b[1]);
    }
    int drop=0;for(int i=1;i<3;++i)if(std::abs(normal[i])>std::abs(normal[drop]))drop=i;
    ...
    for(size_t i=0;i<points.size();++i){const auto& a=points[i];const auto& b=points[(i+1)%points.size()];
      area+=a[0]*b[1]-b[0]*a[1];scale=std::max(scale,std::max(std::abs(a[0]-points[0][0]),std::abs(a[1]-points[0][1])));}
    const double epsilon=std::max(1e-16,scale*scale*1e-10);
    if(std::abs(area)<=epsilon)continue;
    bool simple=true;
    for(size_t i=0;i<points.size()&&simple;++i)for(size_t j=i+1;j<points.size();++j){
      if(j==i+1||(i==0&&j+1==points.size()))continue;
      const auto& a=points[i];const auto& b=points[(i+1)%points.size()];
      const auto& c=points[j];const auto& d=points[(j+1)%points.size()];
      const double ab_c=cross(a,b,c),ab_d=cross(a,b,d),cd_a=cross(c,d,a),cd_b=cross(c,d,b);
      // Reject crossings and collinear contacts; they are not a simple rim.
    ...
    if(!simple)continue;
    std::vector<size_t> polygon(ring.size());std::iota(polygon.begin(),polygon.end(),0u);
    ...
    while(polygon.size()>2){
      ...
      for(size_t i=0;i<polygon.size();++i){
        ...
        if(std::abs(turn)<=epsilon&&(points[b][0]-points[a][0])*(points[b][0]-points[c][0])
            +(points[b][1]-points[a][1])*(points[b][1]-points[c][1])<=epsilon){
          // Retain a flat ear too: its three original weights can bend apart
          // during animation, so dropping its middle rim vertex could crack.
          triangles.push_back({ring[a],ring[b],ring[c]});polygon.erase(polygon.begin()+i);clipped=true;break;
        }
        if(turn<=epsilon)continue;
        bool inside=false;
        for(auto p:polygon)if(p!=a&&p!=b&&p!=c&&sign*cross(points[a],points[b],points[p])>=-epsilon
            &&sign*cross(points[b],points[c],points[p])>=-epsilon&&sign*cross(points[c],points[a],points[p])>=-epsilon){inside=true;break;}
        if(inside)continue;
        triangles.push_back({ring[a],ring[b],ring[c]});polygon.erase(polygon.begin()+i);clipped=true;break;
      }
      if(!clipped)break;
