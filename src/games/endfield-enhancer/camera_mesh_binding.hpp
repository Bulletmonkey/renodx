...
// Shared policy for filtered meshes and verified meshes with no head geometry.
// Own only false->true changes, and restore before releasing a renderer root.
inline void UpdateSkinningPolicy(uint32_t renderer,bool active,bool* owned) {
  if(!NativeAlive(renderer)){*owned=false;return;}
...
inline void RestoreBinding(Binding* state) {
  auto& binding=*state;
  if(!NativeAlive(binding.renderer)){binding.applied=false;binding.owns_offscreen_update=false;return;}
...
inline void UpdateBinding(bool requested) {
  // Clothing/gloves need the same animation updates even if their source mesh
  // had no head triangles to remove. Never replace their mesh or shadow proxy.
  for(auto& item:seen){
    try{
      ...
      try{UpdateSkinningPolicy(item.renderer,false,&item.owns_offscreen_update);}catch(...){}
    }
  }
  ...
  try{
    void* r=gc_target(binding.renderer);
    // Validate pooled rigs before accepting an already assigned copy.
