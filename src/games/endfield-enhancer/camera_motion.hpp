...
// Included inside camera::detail, after movement. Only the rendered camera is
// written here; skeleton animation, IK, and gameplay input remain game-owned.
namespace motion {
...
inline void Capture(void* brain, bool active, bool preserve_direction, const Quat& control_view) {
  ...
  if (!camera_root) return;
  // Capture the unmodified control orientation, never the rendered head pose.
  ...
  if (!reported) { reported=true; Log(reshade::log::level::info,"Endfield enhancer: animated head camera capture active"); }
}

// Mesh bind poses map the model's authored axes into each bone's local space.
// This avoids assuming that the head/chest bone uses Unity's +Z forward axis,
// or calibrating against whatever animation happens to play when enabled.
inline void ResolveAxes() {
  ...
  for (int depth=0; ancestor.Get() && depth<8; ++depth) {
    ...
    if (token.find(L"spine") != std::wstring::npos || token == L"chest") {
      body_root = gc_new(ancestor.Get(),false); break;
    }
  ...
  for (uintptr_t i=0; i<std::min<uintptr_t>(array_length(renderers.Get()),256) && !(head_axes && body_axes); ++i) {
    ...
    for (uintptr_t j=0;j<count;++j) {
      ...
      if (head) { head_forward=forward; head_up=up; head_axes=true; }
      if (body) { body_forward=forward; body_up=up; body_axes=true; }
    }
  }
  Log(reshade::log::level::info,(std::string("Endfield enhancer: camera animation axes: head=")
      +(head_axes?"ready":"unavailable")+" body="+(body_axes?"ready":"unavailable")).c_str());
}
...
inline void Apply(void* camera) {
  ...
  try {
    ...
    if (!keep_direction && (use_head ? head_axes : v.body_facing && body_axes)) {
      ...
      Quat facing{};
      if (Unit(bone) && FacingRotation(Rotate(bone,use_head?head_forward:body_forward),
                                      Rotate(bone,use_head?head_up:body_up),&facing)) {
        // Remove the presentation skeleton's heading. Otherwise an idle look
        // limit replaces mouse yaw with held body yaw, and sidesteps steer the
        // view. Only the local animation belongs on top of the control view.
    ...
    if (keep_direction || !(v.body_facing || v.head_facing || v.realism) || v.animation_motion<=0.f) {
      ...
      // Re-evaluate from the same frame-start pose at TailLateTick. This uses
      // the final animation sample without applying a second smoothing step.
      animation_offset = BlendRotation(frame_start_offset,offset,frame_alpha);
    }
    // Smoothing is restricted to animation. Mouse/controller look is applied
    // fresh on every frame, independent of animation strength or body heading.
    view = input_view * animation_offset;
    ...
    const Vec3 position = eyes + (full_motion ? Rotate(view,{0,v.eye_height,v.eye_forward})
        : Vec3{0,v.eye_height,0}+planar*v.eye_forward) + Vec3{0,v.height,0}
    ...
    wrote_view = true;
    static bool reported = false;
    if (!reported) { reported=true; Log(reshade::log::level::info,"Endfield enhancer: animated head camera update active"); }
  } catch (...) {
    // Missing/replaced rigs fall back to the existing gameplay camera.
    static bool reported = false;
    if (!reported) { reported=true; Log(reshade::log::level::info,"Endfield enhancer: animated head camera update rejected a managed call"); }
  }
}
...
inline void HookedTailTick(void* manager, float dt, MethodInfo* method) {
  ...
  if (camera_root && captured_frame>=0) {
    ...
    static bool reported = false;
    if (!reported) { reported=true; Log(reshade::log::level::info,"Endfield enhancer: tail-late head camera update active"); }
  }
}
