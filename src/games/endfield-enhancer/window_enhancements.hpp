...
namespace endfield::window_enhancements
{
        ...
        DWORD_PTR)
    {
        ...
        case WM_TIMER:
            ...
            {
                if (cursor_guard::GameFocused()) restore_cursor_visibility();
                else release_background_cursor();
                return 0;
            }
            break;
        case WM_ACTIVATEAPP:
        ...
        case WM_STYLECHANGING:
        {
            ...
            if (wparam == static_cast<WPARAM>(GWL_STYLE) && lparam != 0)
            {
                ...
                // Unity reapplies its own captioned popup style during startup.
        ...
        // Let WM_SIZE and WM_EXITSIZEMOVE reach Unity in their original order.
        // Deferring WM_SIZE can leave the render surface at its old dimensions.
    ...
    bool enable_native_resize(HWND window) noexcept
    {
        ...
        // Marshal installation onto the window's thread; never replace GWLP_WNDPROC.
        HMODULE module = nullptr;
    ...
    void toggle_fullscreen(audio_overlay_state &state) noexcept
    {
        ...
        if (windowed)
        {
            ...
            if (!SetWindowLongPtrW(state.game_window, GWL_STYLE,
                    (style & ~(WS_OVERLAPPEDWINDOW | WS_MAXIMIZE | WS_MINIMIZE)) | WS_POPUP) && GetLastError() != ERROR_SUCCESS) return;
        ...
        {
            SetLastError(ERROR_SUCCESS);
            if (!SetWindowLongPtrW(state.game_window, GWL_STYLE, state.saved_window
                    ? state.saved_style : (style & ~WS_POPUP) | WS_OVERLAPPEDWINDOW)
            ...
            {
                // A game launched fullscreen has no previous window placement.
        ...
        LPARAM lparam)
    {
        ...
        case WM_MOUSEMOVE:
        {
            ...
            if (button_hovered != state->button_hovered ||
                slider_hovered != state->slider_hovered || fullscreen_hovered != state->fullscreen_hovered)
            {
        ...
        case WM_LBUTTONUP:
        {
            ...
            const bool activate_fullscreen = state->fullscreen_pressed &&
                point_in_rect(get_fullscreen_rect(*state), point);
    ...
    void update_audio_overlay_position(audio_overlay_state &state) noexcept
    {
        ...
        if (fullscreen)
        {
            ...
            if (GetForegroundWindow() != state.game_window ||
                (!state.slider_dragging && !state.fullscreen_pressed &&
                    (!GetCursorPos(&cursor) || !point_in_rect(reveal, cursor))))
            {
                hide_audio_overlay(state);
                return;
            }
    ...
    {
        // Keep callbacks mapped until the worker and its COM/GDI objects are gone.
...
namespace endfield::window_enhancements
{
    void install_window_enhancements(HWND game_window, HMODULE module) noexcept
    {
        ...
        if (IsWindow(g_enhancements.game_window) &&
    ...
    void uninstall_window_enhancements() noexcept
    {
        ...
        {
            // Swapchain destruction may run on the game window's thread. Service
            // sent Win32 messages so an in-flight style/position change can finish.
