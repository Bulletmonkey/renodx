// Runtime mesh preparation; included inside camera::detail.
namespace mesh_runtime {
struct Pending { uint32_t buffer=0, mesh=0; int bytes=0; std::string file; };
...
// This alias never owns the native descriptor or GPU allocation. Clear m_Ptr
// before dropping its root, so GraphicsBuffer's finalizer cannot release either.
struct ReadAlias {
  alignas(8) std::array<uint8_t,0x58> native{};
  uint32_t root=0;
  ~ReadAlias(){if(root){Write<void*>(gc_target(root),pointer_offset,nullptr);gc_free(root);}}
};
...
inline bool ResetTarget(bool keep_prepared=false) {
  try{
    ...
    if(!keep_prepared)for(auto& b:bindings){
      ...
      for(auto root:{b.renderer,b.source,b.filtered,b.proxy,b.model,b.bones})if(root)gc_free(root);
      b={};
    }
    if(!keep_prepared)bindings.clear();
    for(auto& item:pending){Invoke(dispose,gc_target(item.buffer));gc_free(item.buffer);gc_free(item.mesh);}pending.clear();
    for(auto& item:candidates)for(auto root:{item.mesh,item.renderer,item.model,item.bones})if(root)gc_free(root);
    if(!keep_prepared){
      for(auto& item:seen)for(auto root:{item.renderer,item.mesh,item.bones,item.model})if(root)gc_free(root);
      seen.clear();for(auto root:recent_models)gc_free(root);recent_models.clear();
    }
    last_scan=0;settling_until=GetTickCount64()+1500;
    candidates.clear();captures.clear();
    if(session_model)gc_free(session_model);session_model=0;started=false;
    return true;
  }catch(...){reset_failed=true;static_cast<void>(0);return false;}
}
inline bool PollOne() {
  if(candidates.empty())return false;
  // Finish this mesh before reading lower-priority LODs. Previously every
  // buffer for every LOD was read before any visible mesh could be replaced.
  ...
  if(next==pending.end()){
    auto item=std::move(candidates.front());candidates.erase(candidates.begin());
    candidate_source=item.mesh;candidate_renderer=item.renderer;candidate_model=item.model;
    candidate_bones=item.bones;
    candidate_body_skin=item.body_skin;candidate_dedicated_head=item.dedicated_head;candidate_name=std::move(item.name);candidate_head=std::move(item.head);candidate_neck=item.neck;BuildDetachedMesh();
    if(candidates.empty())captures.clear();
    return true;
  }
  const auto item=*next;pending.erase(next);
  ...
  if(root){
    try{
      ...
      if(item.file.find("-stream")!=std::string::npos){
        ...
        // Shared allocations carry Index as well as Vertex. Remove Index only
        // from this private read descriptor; backing GPU flags stay untouched.
  ...
  Invoke(dispose,gc_target(item.buffer));gc_free(item.buffer);gc_free(item.mesh);
  return false;
}
inline void Poll() {
  // Bound total work by time. Several tiny head meshes can finish in one
  // frame; a costly mesh stops the batch immediately after it completes.
  const auto begin=GetTickCount64();
  for(int i=0;i<12&&!candidates.empty();++i){
    PollOne();
    if(GetTickCount64()-begin>=4)break;
  }
}
inline void Start(bool requested_fill) {
  // Retire copies with the previous cap setting on the game thread. Both the
  // visible and shadow assignments are restored before those copies retire.
  ...
  // Keep up to four prepared character models. The visible bindings are
  // restored on departure; returning only reapplies the verified mesh copy.
  auto recent=std::find_if(recent_models.begin(),recent_models.end(),[&](uint32_t root){return gc_target(root)==gc_target(model_root);});
  if(recent!=recent_models.end()){auto root=*recent;recent_models.erase(recent);recent_models.push_back(root);}
  else {auto root=gc_new(gc_target(model_root),false);if(!root)return;recent_models.push_back(root);}
  if(recent_models.size()>4){gc_free(recent_models.front());recent_models.erase(recent_models.begin());}
  ...
  for(auto it=bindings.begin();it!=bindings.end();){
    if(!it->failed&&retained(it->model)&&NativeAlive(it->renderer)&&NativeAlive(it->source)&&NativeAlive(it->filtered)){++it;continue;}
    RestoreBinding(&*it);
    if(destroy_cached&&NativeAlive(it->filtered))destroy_cached(gc_target(it->filtered),0.f);
    for(auto root:{it->renderer,it->source,it->filtered,it->proxy,it->model,it->bones})if(root)gc_free(root);
  ...
  // Empty/loading hierarchies are retried; no permanent first-frame latch.
  if(!started)static_cast<void>(0);
  if(!ResolveExport(GetModuleHandleW(L"GameAssembly.dll"),"il2cpp_resolve_icall",&resolve))return;
  if(!started&&endfield::runtime_status::LoadedModuleFileSha256(GetModuleHandleW(L"UnityPlayer.dll"))
      !="41d8ba3111c5652c777124ee321d53f14f31ee866a6317eefb4ecf20168370b5"){
    static_cast<void>(0);return;
  }
  ...
  auto buffer_offset=reinterpret_cast<int(*)(void*)>(resolve("UnityEngine.GraphicsBuffer::get_offset"));
  ...
  auto vertex_stride=reinterpret_cast<int(*)(void*,int)>(resolve("UnityEngine.Mesh::GetVertexBufferStride"));
  auto attr_offset=reinterpret_cast<int(*)(void*,int)>(resolve("UnityEngine.Mesh::GetVertexAttributeOffset"));
  ...
  if(pointer_offset!=0x10){static_cast<void>(0);return;}
  ...
  auto get_enabled=FindMethod(core,"UnityEngine","Renderer","get_enabled",0);
  ...
  if(!new_object||!buffer_class||!get_data||!buffer_offset||!new_array||!byte_class||!index_buffer||!vertex_stride||!attr_offset||!dispose||!get_mesh||!get_buffer||!get_attribute||!get_attr_count||!get_vertices||!is_child||!get_parent||!get_bones||!get_count||!get_stride||!get_enabled||!assignable||!skinned){static_cast<void>(0);return;}
  ...
  if(!ResolveExport(module,"il2cpp_class_get_type",&class_type)
      ...
      ||!ResolveExport(module,"il2cpp_array_length",&array_length)){static_cast<void>(0);return;}
  const auto type_root=gc_new(type_object(class_type(skinned)),false);
  if(!type_root){static_cast<void>(0);return;}
  bool inactive=true;void* component_args[]={gc_target(type_root),&inactive};
  ...
  gc_free(type_root);
  if(!array_root){static_cast<void>(0);return;}
  started=true;
  // Only the direct, semantically named neck ancestor qualifies. Do not use
  // IsChildOf(neck): other rigs may place shoulders beneath that transform.
  struct NeckRoot {
    uint32_t handle=0;
    ~NeckRoot(){if(handle)gc_free(handle);}
  } neck;
  if(void* parent=Invoke(get_parent,gc_target(head_root))){
    ...
    if(token!=L"neck"&&neck.handle){gc_free(neck.handle);neck.handle=0;}
  }
  // Preserve explicit proxy references. Endfield also has dedicated proxy
  // renderers whose role is present in their asset names, checked below.
  ...
  for(uintptr_t r=0;r<std::min<uintptr_t>(array_length(gc_target(array_root)),512);++r){
    ...
    if(proxy){auto root=gc_new(proxy,false);if(root)shadows.push_back(root);}
  }
  int captured=0;
  ...
  for(uintptr_t r=0;r<std::min<uintptr_t>(array_length(gc_target(array_root)),512);++r){
    ...
    void* current=Invoke(get_mesh,renderer);if(!current)continue;
    if(std::any_of(shadows.begin(),shadows.end(),[&](uint32_t root){return gc_target(root)==current;}))continue;
    bool known=false;
    for(auto& b:bindings)if(!b.failed&&gc_target(b.renderer)==renderer){
      if(!PaletteMatches(renderer,b.bones)){
        RestoreBinding(&b);b.failed=true;current=Invoke(get_mesh,renderer);
      }else if(!b.failed&&(current==gc_target(b.source)||current==gc_target(b.filtered)))known=true;
    }
    if(known)continue;
    for(const auto& item:seen)if(gc_target(item.renderer)==renderer&&gc_target(item.mesh)==current&&PaletteMatches(renderer,item.bones)){known=true;break;}
    if(known)continue;
    // Endfield submits these renderers itself even when Renderer.enabled is false.
    // Record the flag as evidence; never use it to filter the mesh inspection.
    void* enabled=Invoke(get_enabled,renderer);
    const int enabled_value=enabled?int(*static_cast<bool*>(object_unbox(enabled))):-1;
    ...
    std::string label(wide.begin(),wide.end());label+="-"+std::to_string(r);

    void* mesh=Invoke(get_mesh,renderer);if(!mesh)continue;
    const auto mesh_root=gc_new(mesh,false);if(!mesh_root)continue;
    void* vertices=Invoke(get_vertices,gc_target(mesh_root));
    void* attr_count=Invoke(get_attr_count,gc_target(mesh_root));
    if(!vertices||!attr_count){gc_free(mesh_root);continue;}
    const int vertex_count=*static_cast<int*>(object_unbox(vertices));
    const int attributes=*static_cast<int*>(object_unbox(attr_count));

    if(vertex_count<=0||vertex_count>200000||attributes<1||attributes>16){gc_free(mesh_root);continue;}
    unsigned streams=0;
    for(int i=0;i<attributes;++i){
      void* args[]={&i};void* result=Invoke(get_attribute,gc_target(mesh_root),args);if(!result)continue;
    ...
    Candidate selected;selected.name=label;selected.dedicated_head=endfield::camera::mesh::IsDedicatedHeadMesh(role);selected.body_skin=endfield::camera::mesh::IsBodyMesh(role);
    ...
    if(bones){
      ...
      if(bone_root){
        ...
        for(uintptr_t i=0;i<n;++i){
          ...
          if(candidate)selected.head[i]=attached?endfield::camera::mesh::kHeadBone:
              (neck.handle&&bone==gc_target(neck.handle)?endfield::camera::mesh::kNeckBone:0);

        }
        selected.bones=bone_root;
      }
    }
    // Locate the neck in this mesh's bind space. Clothing can omit the neck
    // from its palette, so use the closest palette ancestor instead. Converting
    // through that bone's current space cancels the character's world transform.
    if(neck.handle&&selected.bones){
      ...
      for(int depth=0;ancestor&&depth<16;++depth){
        ...
        if(index<n&&inverse_point&&bind_method){
          void* poses=Invoke(bind_method,gc_target(mesh_root));uint32_t pose_root=poses?gc_new(poses,false):0;
          if(pose_root&&array_length(gc_target(pose_root))==n){
            std::array<float,16> m;std::memcpy(m.data(),static_cast<uint8_t*>(gc_target(pose_root))+0x20+index*64,64);
            Vec3 head_world{},neck_world{},head_local{},neck_local{};
            position_injected(gc_target(head_root),&head_world);position_injected(gc_target(neck.handle),&neck_world);
            inverse_point(gc_target(ancestor),&head_world,&head_local);inverse_point(gc_target(ancestor),&neck_world,&neck_local);
        ...
        void* parent=Invoke(get_parent,gc_target(ancestor));uint32_t next=parent?gc_new(parent,false):0;
        gc_free(ancestor);ancestor=next;
      }
    ...
    if(!selected.dedicated_head&&selected.neck.length==0&&std::none_of(selected.head.begin(),selected.head.end(),[](uint8_t v){return v!=0;})){
      ...
      }else if(selected.bones)gc_free(selected.bones);
      gc_free(mesh_root);continue;
    }
    selected.mesh=gc_new(gc_target(mesh_root),false);selected.renderer=gc_new(renderer,false);selected.model=gc_new(gc_target(model_root),false);
    if(!selected.mesh||!selected.renderer||!selected.model){
      for(auto root:{selected.mesh,selected.renderer,selected.model,selected.bones})if(root)gc_free(root);
      gc_free(mesh_root);continue;
    }
    const size_t queued_before=pending.size();
    size_t required_buffers=1;for(int i=0;i<4;++i)if(streams&(1u<<i))++required_buffers;
    for(int stream=-1;stream<4;++stream){
      ...
      }catch(...){static_cast<void>(0);continue;}
      if(!buffer){static_cast<void>(0);continue;}
      const auto root=gc_new(buffer,false);if(!root)continue;
      ...
      if(bytes<=0||bytes>8*1024*1024){static_cast<void>(0);Invoke(dispose,gc_target(root));gc_free(root);continue;}
      Pending item{};item.buffer=root;item.mesh=gc_new(gc_target(mesh_root),false);item.bytes=static_cast<int>(bytes);item.file=label+(stream<0?"-indices":"-stream"+std::to_string(stream))+".bin";
      if(!item.mesh){Invoke(dispose,gc_target(root));gc_free(root);continue;}
      if(queued_bytes+item.bytes>128*1024*1024){Invoke(dispose,gc_target(root));gc_free(root);gc_free(item.mesh);static_cast<void>(0);continue;}
      queued_bytes+=item.bytes;pending.push_back(item);
  ...
  gc_free(array_root);
  // Priority only: a missing/renamed LOD suffix cannot change what is hidden.
  ...
  if(!pending.empty())static_cast<void>(0);
}
